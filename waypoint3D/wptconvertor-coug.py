...
def main():
    ...
    description = input('Please type now in the terminal a brief description of the mission: ')
    ...
    # First, we set our base coordinates.  This will serve as our zero/zero
    baseLon = Waypoints[0].longitude
    baseLat = Waypoints[0].latitude

    print(baseLon, baseLat)

    # Second, we calculate the waypoints as meters
    # I will use Haversine's formula, defined in a function above.
    # I wrote the function a long time ago, so, I just trust that it works.
    numCoords = 0
    coords = []
    for waypoint in Waypoints:
        x, y = CalculateHaversine(baseLat, baseLon, waypoint.latitude, waypoint.longitude)
        x = round(x)
        y = round(y)
    ...
    # ceate and execute shell commands to copy .bhv file to a new directory in the docker container (cougars)
    # useful strings
    name_of_container = 'cougars'
    path_in_container = '/home/frostlab/ros2_ws/moos_missions'
